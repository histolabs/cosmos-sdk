...
```go reference
https://github.com/cosmos/cosmos-sdk/v2/blob/v0.50.0-alpha.0/types/tx_msg.go#L91-L95
```

A standard implementation of both these objects can be found in the [`auth/tx` module](../../build/modules/auth/2-tx.md):

```go reference
https://github.com/cosmos/cosmos-sdk/v2/blob/v0.50.0-alpha.0/x/auth/tx/decoder.go
```

```go reference
https://github.com/cosmos/cosmos-sdk/v2/blob/v0.50.0-alpha.0/x/auth/tx/encoder.go
```
...
```go reference
https://github.com/cosmos/cosmos-sdk/v2/blob/v0.50.0-alpha.0/types/account.go#L15-L32
```
...
* the [`Validator`](https://github.com/cosmos/cosmos-sdk/v2/blob/v0.50.0-alpha.0/x/staking/types/staking.pb.go#L340-L377) struct that contains information about a validator.
...
```go reference
https://github.com/cosmos/cosmos-sdk/v2/blob/v0.50.0-alpha.0/x/staking/types/validator.go#L41-L64
```

#### `Any`'s TypeURL

When packing a protobuf message inside an `Any`, the message's type is uniquely defined by its type URL, which is the message's fully qualified name prefixed by a `/` (slash) character. In some implementations of `Any`, like the gogoproto one, there's generally [a resolvable prefix, e.g. `type.googleapis.com`](https://github.com/gogo/protobuf/blob/b03c65ea87cdc3521ede29f62fe3ce239267c1bc/protobuf/google/protobuf/any.proto#L87-L91). However, in the Cosmos SDK, we made the decision to not include such prefix, to have shorter type URLs. The Cosmos SDK's own `Any` implementation can be found in `github.com/cosmos/cosmos-sdk/v2/codec/types`.
...
```go reference
https://github.com/cosmos/cosmos-sdk/v2/blob/v0.50.0-alpha.0/codec/types/interface_registry.go#L28-L75
```
