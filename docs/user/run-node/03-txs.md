...
:::warning
Please note that signing a transaction with multiple signers or with a multisig account, where at least one signer uses `SIGN_MODE_DIRECT`, is not yet possible. You may follow [this Github issue](https://github.com/cosmos/cosmos-sdk/v2/issues/8141) for more info.
:::
...
```go
import (
	"github.com/cosmos/cosmos-sdk/v2/simapp"
)
...
```go reference
https://github.com/cosmos/cosmos-sdk/v2/blob/v0.50.0-alpha.0/client/tx_config.go#L33-L50
```
...
We set encoding config to use Protobuf, which will use `SIGN_MODE_DIRECT` by default. As per [ADR-020](https://github.com/cosmos/cosmos-sdk/v2/blob/main/docs/architecture/adr-020-protobuf-transaction-encoding.md), each signer needs to sign the `SignerInfo`s of all other signers. This means that we need to perform two steps sequentially:
...
import (
	...
	xauthsigning "github.com/cosmos/cosmos-sdk/v2/x/auth/signing"
)
...
import (
	...
	"github.com/cosmos/cosmos-sdk/v2/types/tx"
)
